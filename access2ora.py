...
def logger_setup(logdir):
    ...
    debuglog = logging.FileHandler(os.path.join(
        logdir, datetime.now().strftime('%Y-%m-%d.log')))
...
def push2Ora(conn, mapping, table_column_dict, LOG):
    ora_connection = cx_Oracle.connect(
        config.oracle['username'], config.oracle['password'], config.oracle['url'], encoding="UTF-8")
    cursor = conn.cursor()
    for access_table_name in table_list:
        ...
        LOG.info("We will insert %d records to table %s" %
                 (len(rows), mapping[access_table_name]))
...
def generateTables(conn, mapping, table_column_dict, LOG):
    ...
    table_sql = ''
    for access_table_name in table_list:
        LOG.debug("Working with table %s for mapping and columns info" %
                  (access_table_name))
        ...
        LOG.debug('Table Mapping %s => %s' %
                  (access_table_name, new_table_name))
        column_count = 0
        column_names = []
        for column in cursor.columns(table=access_table_name):
            ...
            LOG.debug('MAP table [%s] - %s [%s] => %s' % (access_table_name,
                                                          column.column_name, column.type_name, new_column_name))
...
def main():
    ...
    parser.add_argument(
        "--sql_only", help='Mode, sql is for generation sqls only and data is for inserting data into oracle', action="store_true")
    args = parser.parse_args()

    LOG = logger_setup("logs")
    MDB = config.access['file_location']
    conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=%s;uid=%s;pwd=%s' %
                          (MDB, config.access['username'], config.access['password']))
